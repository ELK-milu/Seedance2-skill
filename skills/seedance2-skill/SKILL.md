...
用户需提供 ARK API Key 作为参数传入脚本（`--api-key`）。

### 用法

```bash
# 纯文本（2.0 默认模型）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --ratio 16:9 --duration 5 --wait --download ~/Desktop

# 首帧图（有图时 ratio 用 adaptive）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --image img.jpg --ratio adaptive --duration 5 --wait --download ~/Desktop

# 首尾帧
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --image first.jpg --last-frame last.jpg --ratio adaptive --duration 5 --wait --download ~/Desktop

# 视频参考 / 运动复刻（2.0）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --video motion_ref.mp4 --wait --download ~/Desktop

# 音频参考 / 音乐卡点（2.0）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --audio bgm.mp3 --wait --download ~/Desktop

# 多模态混合（图+视频+音频，2.0）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --image img.jpg --video ref.mp4 --audio bgm.mp3 --ratio adaptive --wait --download ~/Desktop

# 自动时长（模型自行决定 4-15 秒，1.5 Pro / 2.0）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --duration -1 --wait --download ~/Desktop

# Draft 样片（低成本预览，确认后再出正片，1.5 Pro）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --image img.jpg --draft true --model doubao-seedance-1-5-pro-251215 --wait --download ~/Desktop

# 离线推理（半价，适合不急的批量任务）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --service-tier flex --wait --download ~/Desktop

# 视频接龙（返回尾帧用于下一段首帧）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --return-last-frame true --wait --download ~/Desktop

# 回调通知（任务完成时 POST 到指定 URL）
python3 scripts/seedance.py --api-key "your-api-key" create --prompt "提示词" --callback-url https://example.com/webhook --download ~/Desktop

# 管理任务
python3 scripts/seedance.py --api-key "your-api-key" status <ID>
python3 scripts/seedance.py --api-key "your-api-key" wait <ID> --download ~/Desktop
python3 scripts/seedance.py --api-key "your-api-key" list --status succeeded
python3 scripts/seedance.py --api-key "your-api-key" delete <ID>
```
